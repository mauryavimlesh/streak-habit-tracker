...
const regex = /<span\s*className=\{cn\(\s*'text-\[11px\] font-medium px-2\.5 py-1 rounded-full shrink-0',\s*task\.completed\s*\?\s*'bg-white\/5 text-white\/40'\s*:\s*task\.priority === 'high'\s*\?\s*'bg-rose-500\/10 text-rose-400 border border-rose-500\/20'\s*:\s*'bg-\[#1a1d25\] text-\[#9ca2b2\] border border-\[#262b36\]'\s*\)\}\s*>\s*\{task\.completed \? 'Done' : task\.type \|\| 'Task'\}\s*<\/span>\s*<\/div>\s*<\/div>/g;

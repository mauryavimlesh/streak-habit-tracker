...
export async function submitFeedback(
  ...
  try {
    ...
    // Also attempt Firestore persistence if configured
    try {
      ...
      await addDoc(collection(db, 'feedback'), {
        ...
        images: newSubmission.images || [],
        email: newSubmission.email || null,
        userId: newSubmission.userId || null,
        createdAt: serverTimestamp(),
      });
    } catch {
      // Local fallback succeeded; ignore firestore offline
    }

...
export default function GoalDetail() {
  ...
  const unhandledPastEntries = Object.values(goal.dailyHistory || {}).filter(entry => 
    entry.date < todayStr && !entry.missedHandled && entry.activities && entry.activities.some(a => !a.completed)
  );

  const missedPastActs = unhandledPastEntries.flatMap(entry => entry.activities!.filter(a => !a.completed));
  ...
  const handleMissedAction = async (action: 'carried_forward' | 'missed' | 'rescheduled', targetDate?: string) => {
    if (!goal || unhandledPastEntries.length === 0) return;

    const updatedHistory = { ...(goal.dailyHistory || {}) };
    
    // Mark past entries as handled
    unhandledPastEntries.forEach(entry => {
      updatedHistory[entry.date] = {
        ...entry,
    ...
    if (action === 'carried_forward' || action === 'rescheduled') {
      const dateToMove = action === 'carried_forward' ? todayStr : (targetDate || todayStr);
      ...
      const uncompletedActs = missedPastActs.map(a => ({
        ...a,
  ...
  return (
    <div className="min-h-screen bg-[#0d0e12] text-white pb-24">
      ...
      <main className="p-4 max-w-2xl mx-auto space-y-6">
        {hasMissedPast && !isRescheduleOpen && (
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 space-y-3">
            <h3 className="text-sm font-bold text-amber-400">Unfinished targets from previous days</h3>
            <p className="text-xs text-amber-400/80">You have {missedPastActs.length} uncompleted activities from past days. What would you like to do with them?</p>
            <div className="flex flex-col gap-2">
              ...
              <button onClick={() => setIsRescheduleOpen(true)} className="w-full py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold text-xs transition-colors">
                Reschedule
              </button>
        ...
        {hasMissedPast && isRescheduleOpen && (
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 space-y-3">
            <h3 className="text-sm font-bold text-amber-400">Reschedule {missedPastActs.length} targets</h3>
            <div>
              <label className="block text-xs text-amber-400/80 mb-1 font-medium">Select new date</label>
              <input
                type="date"
                value={rescheduleDate}
                min={todayStr}
                onChange={e => setRescheduleDate(e.target.value)}
                className="w-full px-3 py-2 rounded-xl bg-black/40 border border-amber-500/20 text-amber-400 text-xs focus:outline-none focus:border-amber-400"
              />
            </div>
            <div className="flex gap-2 mt-2">
              <button onClick={() => setIsRescheduleOpen(false)} className="flex-1 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold text-xs transition-colors">
                Cancel
              </button>
              <button onClick={() => handleMissedAction('rescheduled', rescheduleDate)} className="flex-1 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 font-bold text-xs transition-colors">
                Confirm
              </button>
            </div>
          </div>
